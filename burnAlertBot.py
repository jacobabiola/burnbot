...
ETHERSCAN_API_KEY = 'UFCG3BXXNR58RSNCSFSRYWTNNTG3YW5G7B' #etherscan API key
BSCSCAN_API_KEY = '9QRJJMC5PYM7MTSIEQF8MQKV1H22GU3RGR'
TELEGRAM_BOT_TOKEN = '6527224346:AAHP_gBPJ2_-qKF2j52ZjKXucahFeFNGpDE'
TELEGRAM_CHAT_ID = '@yaratoken'
...
def get_wallet_transactions(wallet_address, blockchain):
    if blockchain == 'eth':
        ...
        url = f'https://scan.pulsechain.com/api?module=account&action=txlist&address={wallet_address}'
    else:
...
def send_telegram_notification(message, value, usd_value, tx_hash, blockchain, w_from, w_to):
    kisa = w_from[:6] + '..' + w_from[-2:]
    if blockchain == 'eth':
        from_wallet = f'<a href="https://etherscan.io/address/{w_from}">{kisa}</a>'
        to = f'<a href="https://etherscan.io/address/{w_to}">Sacrifice</a>'
        etherscan_link = f'<a href="https://etherscan.io/tx/{tx_hash}">Tx hash</a>'
        token= 'ETH'
    elif blockchain == 'bnb':
        blockchain='bsc'
        from_wallet = f'<a href="https://bscscan.com/address/{w_from}">{kisa}</a>'
        to = f'<a href="https://bscscan.com/address/{w_to}">Sacrifice</a>'
        etherscan_link = f'<a href="https://bscscan.com/tx/{tx_hash}">Tx hash</a>'
        token = 'BNB'
    elif blockchain == 'pls':
        value = int(value)
        value = "{:,}".format(value)
        from_wallet = f'<a href="https://scan.pulsechain.com/address/{w_from}">{kisa}</a>'
        to = f'<a href="https://scan.pulsechain.com/address/{w_to}">Sacrifice</a>'
    ...
    message = f'{to} · {blockchain.upper()} \n' \
              f'Received: {value} {token} (~${ausd_value}) From: {from_wallet} \n' \
              f'{etherscan_link}'
...
def monitor_wallets():
    ...
    while True:
        try:
            ...
            eth_usd_price_url = 'https://api.coingecko.com/api/v3/simple/price?ids=ethereum%2Cpulsechain%2Cbinancecoin&vs_currencies=usd'
                ...
                for tx in transactions:
                    ...
                    if tx_hash not in latest_tx_hashes and tx_time > last_run_time:
                        if tx['to'].lower() == wallet_address.lower():
                            ...
                            send_telegram_notification(message, value, usd_value, tx['hash'], blockchain,tx['from'],tx['to'])
...
Example: /add BNB 0x123456789abcdef
Example: /add PLS 0x123456789abcdef
